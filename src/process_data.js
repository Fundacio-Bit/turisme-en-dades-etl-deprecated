    ...
    sheetNames.forEach (sheet => {
        switch (sheet) {
            ...
            case sections[4]:
                ...
                request.makePost(baseUrl, Ambiental.human_pressure_ib(datos, month))
                request.makePost(baseUrl, Ambiental.human_pressure_mallorca(datos, month))
                request.makePost(baseUrl, Ambiental.human_pressure_menorca(datos, month))
                request.makePost(baseUrl, Ambiental.human_pressure_ibiza(datos, month))
                request.makePost(baseUrl, Ambiental.human_pressure_formentera(datos, month))
                break;
            case sections[5]:
                console.log(sheet)
                var datos = XLSX.utils.sheet_to_json(excel.Sheets[sheet]);
